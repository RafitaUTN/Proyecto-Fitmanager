/**
 * Módulo visual de dashboard DashboardChartSection.
 *
 * @remarks Presenta indicadores y gráficos administrativos usando datos agregados de la API.
 */
...
import {
  ...
  useNuevosClientes,
  useDistribucionMembresias,
  useMetodosPago,
  useClientesActivosInactivos,
  useAsistenciasReporte,
  useAsistenciasPorHora,
  useIngresosDiarios,
...
export function DashboardChartSection() {
  ...
  const filterParams = useMemo(() => {
    if (filtros.periodo === 'personalizado') {
      return filtros.fecha_inicio || filtros.fecha_fin
        ? { fecha_inicio: filtros.fecha_inicio || undefined, fecha_fin: filtros.fecha_fin || undefined }
        : undefined
    }
  ...
  const { data: ingresos, isLoading: loadingIngresos } = useIngresosMensuales(filterParams, {
    enabled: modulo === 'ingresos',
  })
  const { data: nuevosClientes, isLoading: loadingClientes } = useNuevosClientes(filterParams, {
    enabled: modulo === 'clientes',
  })
  ...
  const { data: asistencias, isLoading: loadingAsistencias } = useAsistenciasReporte(filterParams, {
    enabled: modulo === 'asistencias',
  })
  const { data: asistenciasPorHora, isLoading: loadingAsisHora } = useAsistenciasPorHora(filterParams, {
    enabled: modulo === 'asistencias',
  })
  const { data: ingresosDiarios, isLoading: loadingIngDiarios } = useIngresosDiarios(filterParams, {
    enabled: modulo === 'ingresos',
  })

  const ingresosData = useMemo(
    () => (ingresos ?? []).map((i) => ({ ...i, label: formatMes(i.mes), total: Number(i.total) })),
    [ingresos],
  )
  const ingresosDiariosData = useMemo(
    () => (ingresosDiarios ?? []).map((i) => ({ ...i, label: formatDia(i.mes), total: Number(i.total) })),
    [ingresosDiarios],
  )
  const clientesData = useMemo(
    () => (nuevosClientes ?? []).map((c) => ({ ...c, label: formatMes(c.mes) })),
    [nuevosClientes],
  )
  const asistenciasData = useMemo(
    () => (asistencias ?? []).map((a) => ({ ...a, label: formatMes(a.mes) })),
    [asistencias],
  )
  const asistenciasHora = useMemo(
    () => (asistenciasPorHora ?? []).sort((a, b) => a.hora - b.hora),
    [asistenciasPorHora],
  )
        ...
        >
          <div data-chart="true" style={cardStyle} className="p-5">
            ...
            {loadingIngresos ? (
              ...
              <div className="h-[340px] flex items-center justify-center text-muted-dark text-sm">
              ...
              <ResponsiveContainer width="100%" height={340}>
                <BarChart data={asistenciasHora} margin={{ top: 20, right: 10, left: 0, bottom: 5 }}>
                  ...
                  <Tooltip
                    ...
                    formatter={(v: any, _: any, props?: { payload?: { hora?: number } }) => [
                      v,
                      props?.payload?.hora != null ? `${props.payload.hora}:00` : '',
                    ]}
                  />
