/**
 * Página Dashboard de la aplicación FitManager.
 *
...
const DashboardChartSection = lazy(() =>
  import('@/features/dashboard/DashboardChartSection').then((m) => ({ default: m.DashboardChartSection })),
)
...
const icons = {
  ...
  dumbbell: (
    ...
    >
      ...
      <path d="M2 12h2" />
      <path d="M20 12h2" />
      <path d="M6 2h1l1 4H6z" />
      <path d="M6 18h1l1 4H6z" />
      <path d="M17 2h-1l-1 4h2z" />
      <path d="M17 18h-1l-1 4h2z" />
    </svg>
  ),
  zap: (
    <svg
...
const sidebarMenus: Record<
  string,
  { id: string; label: string; icon: keyof typeof icons; to: string; disabled?: boolean }[]
> = {
  Administrador: [
...
function Sidebar({ onNavigate }: { onNavigate?: () => void }) {
  ...
  const initials = usuario ? `${usuario.nombre.charAt(0)}${usuario.apellido?.charAt(0) || ''}` : 'AD'

  return (
    <aside
      className="w-[300px] bg-background border-r border-white/[0.08] flex flex-col justify-between h-dvh shrink-0 overflow-hidden"
      style={{ padding: '20px' }}
    >
      <div className="flex flex-col min-h-0">
        ...
        <p className="text-[13px] font-semibold tracking-[2px] text-muted-dark uppercase mt-6 mb-3 shrink-0">
          MENÚ PRINCIPAL
        </p>
        ...
        <nav className="flex-1 space-y-0.5 overflow-y-auto overflow-x-hidden min-h-0">
          {menuItems.map((item) => {
            ...
            const content = (
              ...
              >
                <span className={`shrink-0 ${active ? 'text-white' : 'text-muted-dark'}`}>{icons[item.icon]}</span>
                ...
                {item.id === 'notificaciones' && noLeidas > 0 && (
                  <span className="bg-destructive text-white text-[11px] font-bold w-5 h-5 flex items-center justify-center rounded-full shrink-0">
                    {noLeidas}
                  </span>
                )}
              </div>
            ...
            if (item.to && !item.disabled) {
              return (
                <Link key={item.id} to={item.to} onClick={onNavigate} className="block no-underline">
                  {content}
                </Link>
              )
            }
      ...
      <div className="border-t border-white/[0.08] pt-4 space-y-2 shrink-0">
        ...
        <Link
          ...
          onClick={onNavigate}
          className="flex items-center gap-3 w-full h-12 px-4 text-muted hover:text-foreground hover:bg-surface-light transition-all duration-[250ms] text-base font-medium no-underline"
          style={{ borderRadius: '14px' }}
        >
          {icons.user}
          <span>Mi Perfil</span>
  ...
  return (
    ...
    >
      <div className="flex items-center justify-between">
        <span style={{ color: color || '#F97316' }} className="flex items-center">
          {icon}
        </span>
        {trend && (
...
const MiniStat = memo(function MiniStat({
  ...
  return (
    <div className="flex items-center justify-between py-2 border-b border-white/[0.06] last:border-0">
      ...
      <span className="text-foreground font-semibold text-sm" style={{ color }}>
        {value}
      </span>
    </div>
  )
...
function DashboardAdmin() {
        ...
        <div className="flex-1 space-y-5">
          <div className="grid grid-cols-4 gap-5">
            {[1, 2, 3, 4, 5, 6, 7, 8].map((i) => (
              <div key={i} style={cardStyle} className="h-[110px] animate-pulse" />
            ))}
          </div>
        </div>
      </div>
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.3 }}>
      <div className="mb-6">
        <h1
          className="font-heading text-foreground tracking-wider leading-none"
          style={{ fontSize: 'clamp(32px, 2.8vw, 48px)' }}
        >
          DASHBOARD
        </h1>
...
function DashboardRecepcionista() {
  ...
  const pagosArray = pagosList?.data ?? []
  const clientesArray = clientesList?.data ?? []
  const recientes = pagosArray.slice(0, 4)
  const ultimosClientes = clientesArray.slice(0, 3)
      ...
      <div className="flex flex-col xl:flex-row gap-5 mt-5">
        <div className="flex-1 min-w-0 space-y-5">
          {recientes.length > 0 && (
            <div style={cardStyle} className="p-5">
              <h3 className="text-foreground font-semibold mb-4" style={{ fontSize: '15px' }}>
                Últimos pagos
              </h3>
        ...
        <div className="w-full xl:w-[320px] space-y-5 shrink-0">
          {ultimosClientes.length > 0 && (
            <div style={cardStyle} className="p-5">
              <h3 className="text-foreground font-semibold mb-3" style={{ fontSize: '15px' }}>
                Clientes recientes
              </h3>
              <div className="space-y-2">
                {ultimosClientes.map((c) => (
                  <div key={c.id_cliente} className="flex items-center justify-between py-1">
                    ...
                    <span
                      className={`text-[11px] font-medium px-2 py-0.5 rounded-full ${c.estado ? 'bg-green-500/10 text-green-400' : 'bg-red-500/10 text-red-400'}`}
                    >
...
function DashboardEntrenador() {
  ...
  const misRutinas = (rutinasList ?? []).filter((r) =>
    r.entrenadores?.some((e) => e.id_entrenador === Number(useAuthStore.getState().usuario?.id_usuario)),
  )
    ...
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.3 }}>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-5">
        ...
        <MetricCard
          ...
          value={d?.clientesPresentesHoy ?? 0}
          color="#3B82F6"
        />
                    ...
                    <div className="flex items-center gap-2.5">
                      ...
                      <div>
                        ...
                        <p className="text-muted-dark text-xs">
                          {r._count?.cliente_rutinas ?? 0} clientes · {r._count?.rutina_ejercicios ?? 0} ejercicios
                        </p>
                      </div>
                    </div>
                    <span
                      className={`text-[11px] font-medium px-2 py-0.5 rounded-full ${r.estado ? 'bg-green-500/10 text-green-400' : 'bg-red-500/10 text-red-400'}`}
                    >
...
export function Dashboard() {
  ...
  return (
    <div className="bg-background flex w-full h-dvh overflow-hidden">
      ...
      <div
        className={`fixed inset-y-0 left-0 z-50 lg:static lg:z-auto transition-transform duration-300 ${sidebarOpen ? 'translate-x-0' : '-translate-x-full lg:translate-x-0'}`}
      >
        ...
        >
          <Routes>
            ...
            <Route
              path="clientes"
              element={
                <RoleGuard roles={['Administrador', 'Recepcionista']}>
                  <Clientes />
                </RoleGuard>
              }
            />
