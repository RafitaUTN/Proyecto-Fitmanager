/**
 * Página ClienteMembresia de la aplicación FitManager.
 *
...
export function ClienteMembresia() {
  ...
  const mostrarAlertaPago = data.pago.saldo_pendiente > 0 && data.pago.pago_habilitado && data.dias_restantes <= 5
        ...
        <div className="lg:col-span-2 bg-surface border border-border rounded-card p-6">
          ...
          <div className="mt-6">
            ...
            <p className="text-xs text-muted-dark mt-1.5">
              {data.dias_restantes > 0 ? `${data.dias_restantes} días restantes` : 'Membresía vencida'}
            </p>
          </div>
        </div>
        ...
        <div className="bg-surface border border-border rounded-card p-6">
          ...
          <div className="space-y-3">
            ...
            <div className="flex justify-between text-sm">
              ...
              <span
                className={
                  data.pago.saldo_pendiente > 0 ? 'text-primary font-semibold' : 'text-green-400 font-semibold'
                }
              >
                ₡{data.pago.saldo_pendiente.toLocaleString('es-CR')}
              </span>
            </div>
            {data.pago.estado_pago === 'COMPLETADO' ? (
              <div className="rounded-button bg-green-500/10 p-3 text-sm text-green-400">
                <p className="font-semibold">Pago completado</p>
                <p className="mt-1 text-xs">Tu membresía no tiene saldo pendiente.</p>
              </div>
            ) : mostrarAlertaPago ? (
              <div className="rounded-button bg-primary/10 p-3 text-sm text-primary">
                <p className="font-semibold">Pago pendiente</p>
                <p className="mt-1 text-xs">
                  Tienes un saldo pendiente de ₡{data.pago.saldo_pendiente.toLocaleString('es-CR')} correspondiente a{' '}
                  {plan.nombre}.
                </p>
              </div>
            ) : (
              <div className="rounded-button border border-border bg-surface-light/60 p-3 text-sm text-muted">
                <p className="font-semibold text-foreground">Pago aún no requerido</p>
                <p className="mt-1 text-xs">
                  La alerta de pago se mostrará 5 días antes del vencimiento de la membresía.
                </p>
              </div>
            )}
