/**
 * Página ResetPassword de la aplicación FitManager.
 *
  ...
  return (
    <main className="min-h-dvh bg-background flex items-center justify-center px-4">
      <section className="w-full max-w-md bg-surface border border-border rounded-card p-8">
        ...
        {listo ? (
          <p className="text-muted mt-4">La contraseña se actualizó y las sesiones anteriores fueron revocadas.</p>
        ) : (
          <form onSubmit={submit} className="space-y-4 mt-5">
            ...
            <Button className="w-full" disabled={!token || !valida(password) || password !== confirmacion}>
              RESTABLECER
            </Button>
          </form>
        )}
        <Link to="/login" className="block text-center text-primary text-sm mt-6 hover:underline">
