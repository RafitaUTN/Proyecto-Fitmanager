/**
 * Página Rutinas de la aplicación FitManager.
 *
...
import {
  ...
  useEliminarRutina,
  useAsignarRutina,
  useAsignacionesRutina,
  useClienteRutina,
  useActualizarEjercicioCliente,
} from '@/hooks/use-rutinas'
...
export function Rutinas() {
  ...
  const [editandoEjercicio, setEditandoEjercicio] = useState<{
    id: number
    series: number
    repeticiones: number
    peso: number | null
  ...
  const { data: rutinas, isLoading } = useRutinasPaginadas({
    page: pagina,
    pageSize,
  ...
  const entrenadores = usuariosLista.filter((u: any) => u.rol === 'Entrenador')
  ...
  const asignarMutation = useAsignarRutina(() => {
  ...
  const rutinasPagina = rutinas?.data ?? []
  const totalPaginas = rutinas?.pagination?.totalPages ?? 1
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-end justify-between gap-4">
        <div>
          <h2 className="font-heading text-4xl text-foreground tracking-wider">
            {esAdmin ? 'RUTINAS' : 'MIS RUTINAS'}
          </h2>
          <p className="text-muted mt-1">Diseña planes visuales, ordénalos y asígnalos con claridad.</p>
        </div>
      ...
      <div className="relative">
        ...
        <input
          ...
          placeholder="Buscar rutina por nombre, objetivo o dificultad..."
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 2xl:grid-cols-3 gap-4">
          ...
          >
            <div className="grid grid-cols-3 h-28 bg-surface-light border-b border-border">
              {(r.rutina_ejercicios.length
                ...
                const source =
                  ejercicio.tipo_media === 'animacion'
                    ? ejercicio.animacion_url
                    : ejercicio.imagen_url || ejercicio.animacion_url
                return (
                  <div
                    key={ejercicio.id_ejercicio}
                    className="relative flex items-center justify-center overflow-hidden bg-gradient-to-br from-primary/10 to-surface"
                  >
                    <Dumbbell size={24} className="text-primary/70" aria-hidden="true" />
                    {source && (
                      <img
                        src={source}
                        alt=""
            ...
            <div className="p-5">
              <div className="flex items-start justify-between mb-2">
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2">
                    ...
                    {!r.estado && (
                      <span className="text-xs shrink-0 bg-destructive/10 text-destructive px-2 py-0.5 rounded-badge">
                        Inactiva
                      </span>
                    )}
                    {r.estado && r._count.cliente_rutinas > 0 && (
                      <span className="text-xs shrink-0 bg-secondary/10 text-secondary px-2 py-0.5 rounded-badge">
                        Activa
                      </span>
                    )}
                  </div>
                  ...
                  {r.objetivo && (
                    <p className="flex items-center gap-1.5 text-xs text-muted-dark mt-2">
              ...
              <div className="flex items-center gap-3 text-xs text-muted-dark mt-2">
                ...
                {r.dificultad && (
                  <span className="capitalize rounded-full border border-border px-2 py-0.5">{r.dificultad}</span>
                )}
              </div>
              <div className="flex gap-2 mt-4">
                <Button
                  ...
                  onClick={() => abrirDetailModal(r.id_rutina)}
                >
                ...
                {esAdminOEntrenador && (
                  <>
                    <Button
                      variant="outline"
                      size="sm"
                      onClick={() => abrirEdicionDesdeCard(r.id_rutina)}
        ...
        {!isLoading && rutinasPagina.length === 0 && (
      ...
      {detailModalId && (
        <div
          ...
          onClick={cerrarDetailModal}
        >
          <div className="fixed inset-0 bg-black/60 pointer-events-none" />
          <div
            className="relative bg-surface border border-border rounded-card w-full max-w-4xl shadow-xl"
              ...
              <div className="p-6 space-y-5">
                ...
                <div className="flex items-start justify-between">
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-3">
                      <h3 className="font-heading text-2xl text-foreground tracking-wider truncate">
                        {detalle.nombre}
                      </h3>
                      {!detalle.estado && (
                        <span className="text-xs shrink-0 bg-destructive/10 text-destructive px-2 py-0.5 rounded-badge">
                    ...
                    <div className="flex flex-wrap gap-3 mt-3 text-xs text-muted-dark">
                      ...
                      {detalle.duracion_minutos && (
                        <span className="flex items-center gap-1">
                          <Clock3 size={14} />
                          {detalle.duracion_minutos} min
                        </span>
                      )}
                      {detalle.dificultad && (
                        <span className="capitalize rounded-full border border-border px-2 py-0.5">
                          {detalle.dificultad}
                        </span>
                      )}
                    </div>
                ...
                <div className="flex flex-wrap gap-2">
                  ...
                  <Button
                    size="sm"
                    onClick={() => {
                      setAsignandoClienteId(detalle.id_rutina)
                      setClienteAsignar('')
                    }}
                    disabled={!detalle.entrenadores || detalle.entrenadores.length === 0}
                    title={
                      !detalle.entrenadores || detalle.entrenadores.length === 0
                        ? 'Asigne al menos un entrenador a la rutina'
                        : undefined
                    }
                  >
                    Asignar Cliente
                  ...
                  {esAdmin && (
                    <Button
                      ...
                      variant="outline"
                      onClick={() => {
                        setAsignandoEntrenadorId(detalle.id_rutina)
                        setEntrenadorAsignar('')
                      }}
                    >
                      Asignar Entrenador
                  ...
                  <div>
                    ...
                    <div className="flex flex-wrap gap-2">
                      {detalle.entrenadores.map((e: any) => (
                        <span
                          key={e.id_entrenador}
                          className="flex items-center gap-1.5 text-xs bg-surface-light text-foreground px-3 py-1.5 rounded-badge border border-border"
                        >
                          <svg
                            className="w-3.5 h-3.5 text-muted-dark"
                            viewBox="0 0 24 24"
                            fill="none"
                            stroke="currentColor"
                            strokeWidth="2"
                          ...
                          <button
                            onClick={() =>
                              removerEntrenadorMutation.mutate({
                                idRutina: detalle.id_rutina,
                                idEntrenador: e.id_entrenador,
                              })
                            }
                ...
                <div>
                  ...
                  <div className="grid sm:grid-cols-2 gap-3">
                    {detalle.rutina_ejercicios.map((re: any, index: number) => {
                      const source =
                        re.ejercicio.tipo_media === 'animacion'
                          ? re.ejercicio.animacion_url
                          : re.ejercicio.imagen_url || re.ejercicio.animacion_url
                      return (
                        <article
                          key={re.id_ejercicio}
                          className="flex gap-3 rounded-card border border-border bg-surface-light/60 p-3"
                        >
                          <div className="relative h-20 w-20 shrink-0 overflow-hidden rounded-button bg-primary/10 flex items-center justify-center">
                            <Dumbbell size={24} className="text-primary" />
                            {source && (
                              <img
                                src={source}
                                alt={`Demostración de ${re.ejercicio.nombre}`}
                                loading="lazy"
                          ...
                          <div className="min-w-0 flex-1">
                            <p className="text-[10px] text-muted-dark">#{index + 1}</p>
                            <h5 className="text-sm font-semibold text-foreground truncate">{re.ejercicio.nombre}</h5>
                            <p className="text-xs text-primary">{re.ejercicio.grupo_muscular}</p>
                            <div className="flex flex-wrap gap-x-3 gap-y-1 text-xs text-muted mt-2">
                              <span>{re.series} series</span>
                              <span>{re.repeticiones} reps</span>
                              {re.peso_sugerido && <span>{re.peso_sugerido} kg</span>}
                              {re.descanso !== null && <span>{re.descanso}s descanso</span>}
                            </div>
                            {re.notas && <p className="text-xs text-muted-dark mt-1 line-clamp-2">{re.notas}</p>}
                          </div>
                        </article>
                      )
                    })}
      ...
      {asignandoClienteId && (
        <div
          ...
          onClick={() => setAsignandoClienteId(null)}
        >
          <div
            className="bg-surface border border-border rounded-card p-6 w-full max-w-md space-y-4"
            onClick={(e) => e.stopPropagation()}
          >
            ...
            <div className="flex gap-3">
              <Button onClick={handleAsignarCliente} disabled={!clienteAsignar || asignarMutation.isPending}>
                Asignar
              </Button>
              <Button onClick={() => setAsignandoClienteId(null)} variant="outline">
                Cancelar
              </Button>
            </div>
      ...
      {asignandoEntrenadorId && esAdmin && (
        <div
          className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4"
          onClick={() => setAsignandoEntrenadorId(null)}
        >
          <div
            className="bg-surface border border-border rounded-card p-6 w-full max-w-md space-y-4"
            onClick={(e) => e.stopPropagation()}
          >
            ...
            <div>
              ...
              <select
                value={entrenadorAsignar}
                onChange={(e) => setEntrenadorAsignar(e.target.value)}
                className="w-full rounded-input border border-border bg-surface text-foreground px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-ring"
              >
                <option value="">Seleccionar...</option>
            ...
            <div className="flex gap-3">
              <Button
                ...
                disabled={!entrenadorAsignar || asignarEntrenadorMutation.isPending}
              >
                Asignar
              </Button>
              <Button onClick={() => setAsignandoEntrenadorId(null)} variant="outline">
                Cancelar
              </Button>
            </div>
          </div>
      ...
      {modalOpen && esAdminOEntrenador && (
        ...
        >
          <div className="fixed inset-0 bg-black/60 pointer-events-none" />
          <div
            className="relative bg-surface border border-border rounded-card p-6 w-full max-w-2xl shadow-xl space-y-4"
            onClick={(e) => e.stopPropagation()}
          >
            ...
            <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-[1fr_140px_160px] gap-4">
                <div>
                  ...
                  <input {...register('objetivo')} placeholder="Ej. fuerza de tren superior" className="field" />
                    ...
                    <div key={field.id} className="rounded-card border border-border bg-surface-light/40 p-3 space-y-3">
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-2 items-end">
                        <div className="sm:col-span-2">
                          ...
                          <select
                            {...register(`ejercicios.${index}.id_ejercicio`)}
                          ...
                          {errors.ejercicios?.[index]?.id_ejercicio && (
                            <p className="text-destructive text-[11px] mt-0.5">
                              {errors.ejercicios[index]?.id_ejercicio?.message}
                            </p>
                          )}
                        </div>
                        <div>
                          ...
                          <input
                            type="number"
                            {...register(`ejercicios.${index}.series`)}
                            className="w-full rounded-input border border-border bg-surface text-foreground px-2 py-2 text-xs focus:outline-none focus:ring-2 focus:ring-ring"
                          />
                          {errors.ejercicios?.[index]?.series && (
                            <p className="text-destructive text-[11px] mt-0.5">
                              {errors.ejercicios[index]?.series?.message}
                            </p>
                          )}
                        </div>
                        <div>
                          ...
                          {errors.ejercicios?.[index]?.repeticiones && (
                            <p className="text-destructive text-[11px] mt-0.5">
                              {errors.ejercicios[index]?.repeticiones?.message}
                            </p>
                          )}
                        </div>
                        <div>
                          ...
                          <input
                            type="number"
                            step="0.5"
                            {...register(`ejercicios.${index}.peso_sugerido`)}
                            className="w-full rounded-input border border-border bg-surface text-foreground px-2 py-2 text-xs focus:outline-none focus:ring-2 focus:ring-ring"
                          />
                        </div>
                      </div>
                      <div className="grid grid-cols-1 sm:grid-cols-[140px_1fr] gap-2">
                        <div>
                          ...
                          <input
                            ...
                            min="0"
                            max="3600"
                            {...register(`ejercicios.${index}.descanso`)}
                        ...
                        <div>
                          ...
                          <input
                            ...
                            placeholder="Tempo, postura o variante"
                            className="field text-xs"
                          />
                        </div>
                      </div>
      ...
      {clienteRutinaModal && clienteRutinaDetalle && (
        <div
          className="fixed inset-0 z-50 flex items-start justify-center p-4 pt-8 overflow-y-auto"
          onClick={() => setClienteRutinaModal(false)}
        >
          <div className="fixed inset-0 bg-black/60 pointer-events-none" />
          <div
            className="relative bg-surface border border-border rounded-card p-6 w-full max-w-3xl shadow-xl space-y-4"
            onClick={(e) => e.stopPropagation()}
          >
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-sm text-muted">Rutina: {clienteRutinaDetalle.rutina.nombre}</p>
              </div>
              <button
                onClick={() => setClienteRutinaModal(false)}
                className="text-muted hover:text-foreground text-xl leading-none cursor-pointer bg-transparent border-none"
                ...
                <tbody>
                  {clienteRutinaDetalle.ejercicios?.map((ej: any) => (
                    <tr key={ej.id_cliente_rutina_ejercicio} className="border-t border-border">
                      ...
                      <td className="p-3">
                        <span className="bg-primary/10 text-primary text-xs px-2 py-1 rounded-badge">
                          {ej.grupo_muscular}
                        </span>
                      </td>
      ...
      {ejercicioEditModal && editandoEjercicio && (
        <div
          ...
          onClick={() => setEjercicioEditModal(false)}
        >
          <div className="fixed inset-0 bg-black/60 pointer-events-none" />
          <div
            ...
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setEditandoEjercicio({ ...editandoEjercicio, series: parseInt(e.target.value) || 0 })
                    }
                    className="w-full rounded-input border border-border bg-surface text-foreground px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-ring"
                  />
                </div>
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setEditandoEjercicio({ ...editandoEjercicio, repeticiones: parseInt(e.target.value) || 0 })
                    }
                    className="w-full rounded-input border border-border bg-surface text-foreground px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-ring"
                  />
                </div>
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setEditandoEjercicio({
                        ...editandoEjercicio,
                        descanso: e.target.value ? parseInt(e.target.value) : null,
                      })
                    }
                    className="w-full rounded-input border border-border bg-surface text-foreground px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-ring"
                  />
              ...
              <div>
                ...
                <textarea
                  ...
                  onChange={(e) =>
                    setEditandoEjercicio({ ...editandoEjercicio, observaciones: e.target.value || null })
                  }
                  rows={2}
              ...
              <div className="flex gap-3">
                ...
                <Button onClick={() => setEjercicioEditModal(false)} variant="outline">
                  Cancelar
                </Button>
              </div>
            </div>
