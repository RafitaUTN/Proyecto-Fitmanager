...
const ejercicioSchema = z.object({
  ...
  animacion_url: optionalHttpsUrl.optional(),
...
export function Ejercicios() {
  ...
  function abrirCrear() { reset({ nombre: '', grupo_muscular: '', descripcion: '', nivel: 'principiante', categoria: '', equipo: '', instrucciones: '', imagen_url: '', animacion_url: '', tipo_media: 'imagen', musculos_secundarios: '' }); setEditando(null); setEditorOpen(true) }
  function abrirEditar(item: Ejercicio) {
    setEditando(item)
    reset({ nombre: item.nombre, grupo_muscular: item.grupo_muscular, descripcion: item.descripcion ?? '', nivel: item.nivel as typeof niveles[number], categoria: item.categoria ?? '', equipo: item.equipo ?? '', instrucciones: item.instrucciones ?? '', imagen_url: item.imagen_url ?? '', animacion_url: item.animacion_url ?? '', tipo_media: item.tipo_media ?? 'imagen', musculos_secundarios: item.musculos_secundarios.join(', ') })
    setEditorOpen(true)
  }
  function submit(data: EjercicioForm) {
    const payload = { ...data, imagen_url: data.imagen_url || undefined, animacion_url: data.animacion_url || undefined, musculos_secundarios: data.musculos_secundarios?.split(',').map((item) => item.trim()).filter(Boolean) }
